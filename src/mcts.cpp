...
TreeNode::TreeNode(TreeNode *parent, double p_sa, unsigned int action_size, int max_round, int current_round, int current_step, int step_num_one_round)
    : 
      max_round(max_round),
...
void TreeNode::update_winrate(float winrate)
{
...
unsigned int TreeNode::select_random(double c_puct, double c_virtual_loss) {
  unsigned int best_move = 0;
  TreeNode *best_node;
  double max_value = -DBL_MAX;
  ...
  best_move = *select_randomly1(best_move_list.begin(), best_move_list.end());
...
void TreeNode::expand(const std::vector<double> &action_priors) {
  {
    ...
    if (this->is_leaf) {
      ...
      for (unsigned int i = 0; i < action_size; i++) {
        ...
        this->children[i] = new TreeNode(this, action_priors[i], action_size, max_round, child_current_round, child_current_step, child_step_num_one_round);
      }
...
void TreeNode::backup(double value) {
  ...
  {
      value += static_cast<double>(this->winrate);
  }
  ...
  int total_step = this->current_step + this->step_num_one_round * this->current_round; 
  if (total_step == 0)
  {
    return;
  }
  // update q_sa
  {
    ...
    if(MAIN_PLAYER_LIST[total_step-1] == 0)
...
// MCTS
MCTS::MCTS(predict_model *neural_network, unsigned int thread_num, double c_puct,
    ...
    : c_puct(c_puct),
      ...
      root(new TreeNode(nullptr, 1., action_size, max_round, current_round, current_step, step_num_one_round), MCTS::tree_deleter){}
...
void MCTS::update_with_move(int last_action) {
  ...
  if (last_action >= 0 && old_root->children[last_action] != nullptr) {
    ...
    this->root.reset(new TreeNode(nullptr, 1., this->action_size, this->max_round, new_round, new_step, this->step_num_one_round));
  }
}
...
std::vector<double> MCTS::get_action_probs(Gamecore *gamecore, double temp) { 
...
void MCTS::simulate(std::shared_ptr<Gamecore> game) {
  ...
  while (true) {
    ...
    // select
    auto action = node->select_random(this->c_puct, this->c_virtual_loss);
  ...
  if (status[0] == 0) 
  {
    ...
    //std::cout<<action_priors<<'\n';
    value = result[1][0];
    if (game->get_current_total_step() == 1)
    {
        value = -value;
    }
  ...
  if(node->get_current_step() == 0 && node->get_current_round()>0)
  {
    float tmp_winrate = status[1];
    if (MAIN_ROUND_LIST[game->get_current_round()-1] == 1)
    {
        tmp_winrate = -tmp_winrate;
    }
    node->update_winrate(tmp_winrate);
  }
  if (node->get_current_round() > game->get_max_round())
  {
    value = 0.0;
  }
  /*
